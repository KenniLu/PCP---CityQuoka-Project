exports.handler = async (event) => {
  ...
  if (request.querystring) {
          ...
          if (value && SUPPORTED_FORMATS.includes(value.toLowerCase())) {
            ...
            if (format === 'auto') {
              ...
              if (request.headers && request.headers['accept']) {
                const headerAccept = (request.headers['accept'][0] || {})['value']
                if (headerAccept.includes('avif')) {
                  format = 'avif'
                } else if (headerAccept.includes('webp')) {
                  format = 'webp'
                }
              }
            }
    ...
    if (Object.keys(normalizedOperations).length > 0) {
      ...
      if (normalizedOperations.focus)
        normalizedOperationsArray.push('focus=' + normalizedOperations.focus)
