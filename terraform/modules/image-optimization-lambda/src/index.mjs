...
// Function to validate the focal point format
function validateFocalPoint(focusPoint) {
  const regex = /^(\d{1,2}|100)_(\d{1,2}|100)$/
  return regex.test(focusPoint)
}

// Function to map percentage-based focal point to Sharp's position value
function mapFocalPointToPosition(focusPoint) {
  const [xPercent, yPercent] = focusPoint.split('_').map((val) => parseInt(val, 10))
  const normalizedX = xPercent / 100
  const normalizedY = yPercent / 100
  // Map normalized coordinates (0-1) to Sharp's position constants
  if (normalizedY < 0.33) {
    // Top region
    if (normalizedX < 0.33) return 'left top'
    else if (normalizedX > 0.66) return 'right top'
    else return 'top'
  } else if (normalizedY > 0.66) {
    // Bottom region
    if (normalizedX < 0.33) return 'left bottom'
    else if (normalizedX > 0.66) return 'right bottom'
    else return 'bottom'
  } else {
    // Middle region
    if (normalizedX < 0.33) return 'left'
    else if (normalizedX > 0.66) return 'right'
    else return 'centre' // Sharp uses British spelling for this position
  }
}
...
export const handler = async (event) => {
  ...
  try {
    ...
    if (operationsJSON['height']) {
      ...
      // Set a 3:2 aspect ratio by default
      resizingOptions.height = Math.round(resizingOptions.width * (2 / 3))
    }

    // Handle focal point if specified
    if (operationsJSON['focus'] && validateFocalPoint(operationsJSON['focus'])) {
      resizingOptions.position = mapFocalPointToPosition(operationsJSON['focus'])
      // Set fit to cover to maintain aspect ratio while covering the specified dimensions
      resizingOptions.fit = 'cover'
    }
