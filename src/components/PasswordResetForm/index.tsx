...
const PasswordResetForm: React.FC<PasswordResetFormProps> = ({ token }) => {
  ...
  const onSubmit = async (data: PasswordResetFormValues) => {
    try {
      const { success, error } = await updatePassword(token, data)
      if (error) {
    ...
    <div>
      <Alert variant="default" className="my-2">
        ...
        <AlertDescription className="text-base">
          Password updated successfully. Please login with your account
        </AlertDescription>
      </Alert>
  ...
  return <>{showSuccessMessage ? successMessage() : passwordResetForm()}</>
}
