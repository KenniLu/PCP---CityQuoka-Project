...
// Normalise query string tags so shared aliases (e.g. "near") resolve consistently.
...
// Convert the raw Payload document into the JSON payload returned to the client.
...
export async function GET(request: NextRequest) {
  try {
    ...
    // Cap the max page size to keep the query reasonably small.
    ...
    if (tag) {
      // Payload requires the contains operator for array fields.
      where.tags = {
        contains: tag,
      }
    }
