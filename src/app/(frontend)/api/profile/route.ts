...
const profileSchema = z.object({
  ...
  mobileNumber: z.coerce.number().positive('Mobile number must be a positive number'),
  email: z.string().trim().email('A valid email is required'),
  address: z.string().trim().min(1, 'Address is required'),
  city: z.string().trim().min(1, 'City is required'),
  state: z.string().trim().min(1, 'State is required'),
  postalCode: z.coerce.number().positive('Postal code must be a positive number'),
})
...
const mapUserToProfile = (user: CmsUser) => ({
  ...
  mobileNumber: user.mobileNumber ? Number(user.mobileNumber) : 0,
  email: user.email,
  address: user.address ?? '',
  city: user.city ?? '',
  state: user.state ?? '',
  postalCode: user.postalCode ? Number(user.postalCode) : 0,
})
  ...
  try {
    ...
    const [updatedUser] = await payload.db.drizzle
      ...
      .set({
        ...
        mobileNumber: updates.mobileNumber.toString(),
        email: updates.email,
        address: updates.address,
        city: updates.city,
        state: updates.state,
        postalCode: updates.postalCode.toString(),
